...
def Tree(N):
    layer = N//4
    f_layer = N - layer*3
    block = f_layer - layer + (f_layer - layer==0)
    print(f_layer)
    print(layer)
    for i in range(f_layer):
        for j in range(f_layer+block+2-i,0,-1):
            print(" ", end = "")
        for k in range(-i,i+1):
                print('*', end = "")
        print("")
    for i in range(layer):
        for j in range(f_layer-i,0,-1):
            print(" ", end ="")
        for k in range(-i-(f_layer-3),i+1+f_layer-3):
            print('*', end = "")
        print("")
    for i in range(layer):
        for j in range(f_layer-i-1,0,-1):
            print(" ", end = "")
        for k in range(-i-(f_layer-2),i+1+f_layer-2):
            print('*', end = "")
        print("")
    for i in range(layer):
        for j in range(block+layer+1):
            print(" ", end = "")
