...
def distance(latitude1,longitude1,latitude2,longitude2):
    radius       = 6371.01
    distance     = radius*math.acos(math.sin(math.radians(latitude1))*math.sin(math.radians(latitude2))+math.cos(math.radians(latitude1))*math.cos(math.radians(latitude2))*math.cos(math.radians(longitude1-longitude2)))
    return distance
def area(d1,d2,d3):
    s=(d1+d2+d3)/2
...
print("The area is ",area(d_CS,d_CO,d_SO)+area(d_CO,d_CA,d_AO)," square kilometers")

#Problem7
for i in range(0,2):
    random.seed(time.time())
    print("The random uppercase letter according time.time() is: ",random.choice(string.ascii_uppercase))
